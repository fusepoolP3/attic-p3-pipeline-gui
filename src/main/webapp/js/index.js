    ...
    .done(function(json) {
        ...
        jQuery.each(data.pipelines, function(i, pipeline) {
            ...
            if(first || pipeline.uri == uri){
                ...
                pipelineListContent +='<a href="#" onclick="refreshContainers(\'' + pipeline.uri + '\');" class="list-group-item list-group-item-info"><span class="badge">' + pipeline.transformers.length + '</span><b>' + name + '</b></a>';

                jQuery.each(pipeline.transformers, function(j, transformer) {
                    transformerListContent +='<tr><td>' + (j+1) + '</td><td>' + transformer.name + '</td><td>' + transformer.uri + '</td></tr>';
                });

                first = false;
            }
            else{
                pipelineListContent +='<a href="#" onclick="refreshContainers(\'' + pipeline.uri + '\');" class="list-group-item"><span class="badge">' + pipeline.transformers.length + '</span>' + name + '</a>';
            }
        });
...
function deletePipeline(){
    createConfirmDialog('Confirmation', 'Are you sure you want to delete "<b>' + selectedPipeline.name + '</b>"?', 'deleteConfirmed()');
}
